export default async function acwingScraper(): Promise<InjectResponse<ScrapeData>> {
  try {
    ...
    console.log('[CP2N] getAcwingData', data);
    return {
      data: data
    };
  } catch (e) {
    console.error('[CP2N] acwingScraper error', e);
    return {
      data: null,
      error: e
    };
  } finally {
    console.log('[CP2N] acwingScraper done');
  }
}
